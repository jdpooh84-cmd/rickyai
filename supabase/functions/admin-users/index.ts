...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // C-04: Admin role check via user_roles table (service_role, bypasses RLS)
    ...
    // C-04: Audit helper — logs admin actions for sensitive mutations
    const audit = async (act: string, targetType: string, targetId: string | null, meta?: Record<string, unknown>) => {
      ...
      if (logErr) console.warn("[admin-users] Audit log insert failed:", logErr.message);
    };
    ...
    if (action === "grant_role") {
      ...
      if (error) throw error;
      await audit("grant_role", "user", user_id, { role, granted_by: callerId });
    ...
    if (action === "revoke_role") {
      ...
      const { error } = await supabase
        ...
        .eq("role", role);
      if (error) throw error;
      await audit("revoke_role", "user", user_id, { role, revoked_by: callerId });
      return new Response(JSON.stringify({ success: true }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (action === "update_payout") {
      ...
      const { error } = await supabase
        ...
        .eq("id", payout_id);
      if (error) throw error;
      await audit("update_payout", "affiliate_payout", payout_id, { status });
      return new Response(JSON.stringify({ success: true }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (action === "create_advertiser") {
      ...
      const { data: newAdv, error } = await supabase
        ...
        .single();
      if (error) throw error;
      await audit("create_advertiser", "advertiser_account", newAdv?.id ?? null, { company_name, contact_email, industry });
      return new Response(JSON.stringify({ success: true }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (action === "update_advertiser_status") {
      ...
      const { error } = await supabase
        ...
        .eq("id", advertiser_id);
      if (error) throw error;
      await audit("update_advertiser_status", "advertiser_account", advertiser_id, { status });
      return new Response(JSON.stringify({ success: true }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
