...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // C-04: Audit log — record admin access to stats
    await supabase.from("admin_activity_log").insert({
      admin_user_id: userId,
      action: "view_stats",
      target_type: "platform",
      target_id: null,
      metadata: { endpoint: "admin-stats", method: req.method },
    }).then(({ error }) => {
      if (error) console.warn("[admin-stats] Audit log insert failed:", error.message);
    });

    const [profiles, businesses, referrals, payouts, activeAds, campaigns] = await Promise.all([
      supabase.from("profiles").select("user_id, display_name, created_at", { count: "exact" }),
      supabase.from("businesses").select("id", { count: "exact" }),
      supabase.from("referral_codes").select("*").order("conversions", { ascending: false }).limit(20),
      supabase.from("affiliate_payouts").select("*").eq("status", "pending"),
